...
const services = [
  {
    image: wwdb1,
    title: "IP Surveillance Solution",
    details:
  ...
  },
  {
    ...
    title: "Storage",
    details:
      "When it comes to storing data, services like NAS (Network Attached Storage) and SAN (Storage Area Network) help employees work together and respond to customer needs quickly. Its important for these resources to be accessible and easy to use, all while keeping costs low. Having a plan to deal with any unforeseen circumstances and backing up data is also crucial for an organisation can continue to function with as little disruption as possible. Implementing a business continuity strategy prior to any potential disasters not only saves time and resources but also ensures a swift recovery.",
  },
];
...
export default function Services() {
  ...
  return (
    <div className="services">
      ...
      <p className="service-head" data-aos="fade-up">
      ...
      <div className="services-in" data-aos="fade-up">
        <div>
          ...
          >
            ...
            >
              <div
                className="d-flex services-main"
                style={{
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        transition:
                          currentIndex === index ? "width 5s" : "width 0s",
                      }}
                    ></span>
                  </div>
                ))}
          ...
          <div
            className="services-mainf"
            style={{ padding: "3rem", height: "35rem", marginTop: "1rem" }}
          >
            {currentService && (
              <div>
                <div
                  ref={ref}
                  className="service-content fade-in"
                  style={{ gap: "15px" }}
                >
                  {currentService.title === "IP Surveillance Solution" && (
                    <IPSlider />
                  )}
                  {currentService.title === "IP Networking Solution" && (
                    <INSlider />
                  )}
                  {currentService.title === "Integration Solutions" && (
                    <IntegrationSlider />
                  )}
                  {currentService.title === "Audio-Visual" && (
                    <AudioVisualSlider />
                  )}
                  {currentService.title === "Security & Management" && (
                    <SecuritySlider />
                  )}
                  {currentService.title === "Energy Solutions" && (
                    <EnergySlider />
                  )}
                  {currentService.title === "Surveillance and Safety" && (
                    <SurveillanceSlider />
                  )}
